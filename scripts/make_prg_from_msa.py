...
def get_interval_seqs(interval_alignment):
    ...
    for s in list(
        remove_duplicates(
            [str(record.seq).replace("-", "").upper() for record in interval_alignment]
        )
    ):
    ...
    if len(ret_list) == 0:
        print(
            "Every sequence must have contained an N in this slice - redo sequence curation because this is nonsense"
        )
        ...
        if len(self.consensus.replace("-", "")) < self.min_match_length:
            ...
            for i in range(self.length):
                ...
                if letter != "*":
                    ...
                    match_string = self.consensus[
                        match_start : match_start + match_count
                    ].replace("-", "")
        ...
        if interval[1] - interval[0] <= self.min_match_length:
            ...
            return_id_lists = [
                [
                    record.id
                    for record in self.alignment
                    if str(record.seq[interval[0] : interval[1] + 1]).replace("-", "")
                    == seq
                ]
    ...
    def get_gfa_string(self, prg_string, pre_var_id=None):
        ...
        while str(self.gfa_site) in prg_string:
            ...
            for var_string in vars:
                ...
                var_end_ids = self.get_gfa_string(
...
def main():
    ...
    parser.add_argument(
        "-p", "--prefix", dest="prefix", action="store", help="Output prefix"
    )
    parser.add_argument(
        "-v",
        "--verbosity",
